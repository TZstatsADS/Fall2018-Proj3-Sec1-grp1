---
title: "Project 3 Baseline - Group 1"
author: Chengliang Tang, Tian Zheng, Liu Han, Hongyu Ji, Yi Lin, Zehan Wang, Xiaojie
  Wei
output:
...
This file is currently a template for running evaluation experiments of image analysis (or any predictive modeling). 
```{r,message=FALSE}
if(!require("EBImage")){
...
* Here, we use the same train dataset as our test dataset.We do not create test data by random subsampling from train set.
...
* We choose value 1,2,3 as our interaction depth parameter in GBM.
...
```{r train_label}
extra_label <- read.csv("label.csv")
```
...
```{r runcv, message=FALSE, warning=FALSE, fig.show='hide'}
...
We calculate both MSE and PSNR as performance measurement in image super-resolution.
```{r}
#MSE
